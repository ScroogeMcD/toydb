...
public class ByteUtil {
  ...
  /**
   * How does one convert a character to hex, can be different based on encodings, but conversion
   * from hex to byte, and byte to hex, will be the same across languages.
   */

  /**
   * converts a string of hexadecimal to bytes : one hex character requires 4 bits, so two
   * hex-characters is one byte example : Test => 0x 54 65 73 74
   */
