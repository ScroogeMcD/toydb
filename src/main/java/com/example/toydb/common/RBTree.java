 ...
 *  - Leaf nodes (null) are always considered black
 *  - If a node is Red, its children have to be black (i.e. two red nodes cannot be present one after another in the same hierarchy)
 *  - For each node, all simple paths from the node to the descedant leaves contain the same number of black nodes.
 */
public class RBTree {
    ...
    class Node{
        ...
        public String toString() {
            ...
            return key + "\t:" + value + "\t\t:" + color + "\t:Left(" + leftKey + ")\t:Right(" + rightKey + ")\t:Parent(" + parentKey + ")";
        }
    }
    ...
    public Pair<Data,Data> getInorderSuccessorPredecessor(String key){
        ...
        Data predecessor = predecessorNode.node != null ? new Data(predecessorNode.node.key, predecessorNode.node.value) : null;
        Data successor = successorNode.node != null ? new Data(successorNode.node.key, successorNode.node.value) : null;

        return new Pair<>(predecessor,successor);

    }

    private void getInorderSuccessorPredecessor(String key, Node root, NodeWrapper predecessor, NodeWrapper successor){
        if(root == null) return;
    ...
    /**
     * Insert a node as RED following BST properties, and then recolor the tree, since the red-black property might be violated now.
     * @param node
     */
     ...
     * - CASE 1 : parent and uncle of the inserted node are both red
     * - CASE 2 : parent red, uncle black, and inserted node is right child of parent
     * - CASE 3 : parent red, uncle black, and inserted node is left child of parent
     * @param node
     */
    ...
    /**
     * leftRotates the subtree rooted at X. It assumes that the right subtree Y of X is not null.
     * STEP 01 : Say parent of X is P. If X is left of P, set left of P to Y, else right of P to Y
     * STEP 02 : Set Right of X = Left of Y
     * STEP 03 : Set Left of Y = X
     *
    ...
    /**
     * rightRotates the subtree rooted at Y. It assumes the left subtree X of Y is not null.
     * STEP 01 : change parent of x = parent of y
     * STEP 02 : y.left = x.right
     * STEP 03 : x.right = y
     * @param y
     */
